...
#include "tinyHTTP.h"
#include "../../../../../include/stm32_programmer.h"
            ...
            if (file)
            {
                ...
                //{
                    ...
                    while (http.client.available())
                    {
                        ...
                        onUpdateProgress(n, http.total_len);
                        if(n == http.total_len){
                            break;
                        } 
                    }
...
bool otadrive_ota::syncResources(Client &client)
{
    ...
    while (list.length())
    {
        ...
        else
        {
            ...
            deleteExistingFile();
            // alllow time to erase existign file
            delay(3000);
            ...
            bool ret = download(client, baseurl + "&fk=" + fk, &file, nullptr);
            file.close();
            return ret;
        }
    }
